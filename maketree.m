...
%---------------------------------------------- basic checks    
    if (~isnumeric(rp))
        error('maketree:incorrectInputClass','Incorrect input class.') ;
    end
%---------------------------------------------- basic checks
    if (nargin < +1 || nargin > +2)
        error('maketree:incorrectNumInputs','Incorrect number of inputs. ');
    end
    if (ndims(rp) ~= +2 || mod(size(rp,2),+2) ~= +0)
        error('maketree:incorrectDimensions','Incorrect input dimensions.');
    end
...
%-- main loop : divide nodes until all constraints satisfied
    ...
    %--------------------------- divide node if too populous
        ...
        %----------------------- split plane on longest axis
           [mx,ax] = max(xx(ni,rv)-xx(ni,lv));
          
        %----------------------- push rectangles to children
            il = rd(ll{ni},ax) > mx * op.long;
            lp = ll{ni}( il);       %  "long" rectangles
            ls = ll{ni}(~il);       % "short" rectangles
